...
func (l *Lexer) NextToken() token.Token {
	...
	case '&':
		if l.peekChar() == '&' {
			...
			l.readChar()
			literal := string(ch) + string(l.ch)
			tok = token.Token{Type: token.CONJUNCTION, Literal: literal}
		}
	case '|':
		if l.peekChar() == '|' {
			ch := l.ch
			l.readChar()
			literal := string(ch) + string(l.ch)
			tok = token.Token{Type: token.DISJUNCTION, Literal: literal}
		}
	case 0:
		tok.Literal = ""
		tok.Type = token.EOF
...
func (l *Lexer) read(fn func(ch byte) bool) string {
	position := l.position
	for fn(l.ch) {
		l.readChar()
	}
