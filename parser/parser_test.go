...
func TestReturnStatements(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if !ok {
			t.Errorf("program.Statements[0] not *ast.ReturnStatement. got=%T", stmt)
		}
		...
		if returnStmt.ReturnValue.TokenLiteral() != tt.expectedReturnValue {
			t.Errorf("returnStmt.ReturnValue does not return the correct value. expected=%s, got=%s", tt.expectedReturnValue, returnStmt.ReturnValue.TokenLiteral())
		}
	}
}
